...
namespace LC_FB_Easy
{
    ...
    class FirstUniqueCharInString
    {
        public int FirstUniqChar(string s)
        {
            ...
            // First pass:
            // We will use a char count map to hold the first occurring position (index) of the char
            ...
            for (int i = 0; i < s.Length; i++)
            {
                ...
                if (charMap[charAscii - 65] == 0)
                    charMap[charAscii - 65] = i; // First occurrence
                else
            ...
            for (int i = 0; i < charMap.Length; i++) {
                if (charMap[i] >= 0 && charMap[i] < minIndex) minIndex = charMap[i];
            }
